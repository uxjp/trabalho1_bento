...
void call_Gauss_Jacobi(void *matriz_a, void *vetor_b, int n, double erro){
	int linhas = 0, i, j, k;
	int interacoes = 0, interacoes_total = 0;
	double valor;
	...
	for (i = 0; i < n; i++){
		...
		vetor_linha_inversa = Gauss_Jacobi(matriz_a, vetor_linha_identidade, n, erro, &interacoes);
...
void imprimir_vetor(void *vetor, int n){
	int i;
	printf("[");
	printf("%0.10lf", obter_vetor(vetor, 0));
	for(i = 1; i < n; i++){
...
void *Gauss_Jacobi(void *matriz_a, void *vetor_b, int n, double erro, int *interacoes){
...
void *Gauss_Seidel(void *matriz_a, void *vetor_b, int n, double erro, int *interacoes){
		//Encontra os valores usando o método de Gauss Jacobe e retorna os valores em um vetor
		int i = 0;
		void *vetor_x0;
		void *vetor_xk;
...
void call_Gauss_Seidel(void *matriz_a, void *vetor_b, int n, double erro){
    ...
    printf("\nCALCULO COLUNA %d DA INVERSA:\n\n",i);
		vetor_linha_inversa = Gauss_Seidel(matriz_a, vetor_linha_identidade, n, erro, &interacoes);
